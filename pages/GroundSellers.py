    ...
    if submitted:
        if not selected_event_name or selected_event_name == "No events found" or \
           ...
           not selected_ticket or selected_ticket == "No tickets found":
            ...
            # --- STRICT VAULT CHECK: Find actual physical tickets generated by the Static App ---
            try:
                # Fetch tickets that match the event and type, but haven't been assigned to this vendor yet
                ...
                # Filter for tickets that are sitting in the Vault (Main Gate or unassigned)
                available_tickets = []
                if available_response.data:
                    for t in available_response.data:
                        v_name = str(t.get('vendor_name') or '').strip().lower()
                        # If the ticket belongs to Main Gate, or has no owner, it's available in the vault
                        if v_name in ['', 'none', 'null', 'main gate']:
                            available_tickets.append(t['id'])
                
                if len(available_tickets) < initial_stock:
                    st.error(f"⚠️ Vault Shortage: You requested {initial_stock} tickets, but only {len(available_tickets)} '{clean_tag}' tags are available in the vault. Please generate more tags first.")
                else:
                    # 1. Claim the physical tickets by transferring their ownership in the master database
